...
int main(int argc,char* argv[]) {
  try {  // This allows the program to catch exceptions thrown by ITK code
    ...
    for(patchesIt->GoToBegin();!patchesIt->IsAtEnd();++(*patchesIt)) {
      ...
      sprintf(original_patch_filename,"original_patch%03d.tif",patchesIt->GetCurrentPatchNum());
      ...
      sprintf(resized_patch_filename,"resized_patch%03d.tif",patchesIt->GetCurrentPatchNum());
  ...
  for (int i=0;i<fmin(num_patches,eigenimage_size_x*eigenimage_size_y);i++) {
    ...
    f2icaster->SetInput ( rescaler->GetOutput() );
    inputImageWriter->SetInput( f2icaster->GetOutput() );
    sprintf(eigenface_filename,"eigen_out%03d.tif",i);
  ...
  for (int i=0;i<num_patches;i++) {
    ...
    f2icaster->SetInput ( rescaler->GetOutput() );
    inputImageWriter->SetInput( f2icaster->GetOutput() );
    sprintf(reproj_filename,"reproj.%03d.%03d.tif",i,0);
    ...
    sprintf(reproj_filename,"reproj.%03d.%03d.tif",i,1);
    inputImageWriter->SetFileName( reproj_filename );
    inputImageWriter->Update();
#endif
    ...
    for (int j=1;j<ncomponents;j++) {
      ...
      f2icaster->SetInput ( rescaler->GetOutput() );
      inputImageWriter->SetInput( f2icaster->GetOutput() );
      sprintf(reproj_filename,"reproj.%03d.%03d.tif",i,j+1);
      inputImageWriter->SetFileName( reproj_filename );
      inputImageWriter->Update();
